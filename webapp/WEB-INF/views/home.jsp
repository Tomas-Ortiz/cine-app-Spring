...
<title>Home | Village Cines</title>

<!--/cineapp/resources y /cineapp/  -->
...
<body>
	...
	<!--  Se incluye el menu con la etiqueta include propia jsp -->
	...
	<div class="container theme-showcase" role="main">
		...
		<div id="myCarousel" class="carousel slide" data-ride="carousel">
			...
			<ol class="carousel-indicators">
				<!-- contador es un objeto, contador.index empieza a contar desde 0, y se suma en cada iteracion  -->
				<!-- El primer circulo siempre se activa -->
				<c:forEach items="${banners}" var="banner" varStatus="contador">
					<c:choose>
						<c:when test="${contador.index == 0}">
							<li data-target="#myCarousel" data-slide-to="${contador.index}"
								class="active"></li>
						</c:when>
						<c:otherwise>
							<li data-target="#myCarousel" data-slide-to="${contador.index}"></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</ol>
					...
					<c:choose>
						<c:when test="${contador.index == 0}">
							<div class="item active">
								<img src="${urlPublic}/images/${banner.imagen}" alt="Slide"
									title="${banner.titulo}">
							</div>
						</c:when>
						<c:otherwise>
		...
		<div class="row page-header">
			<div class="col-lg-12">
				...
				<form class="form-inline" action="${urlRoot}search" method="post">
					...
					<!-- Cuando se modifica algun recurso (post, put, delete) se requiere enviar
					el token CSRF (para evitar ataques), de lo contrario se negara el acceso (403)-->
					<!--  cada solicitud requiere, además de la cookie de sesión, un token generado aleatoriamente como parámetro HTTP -->
